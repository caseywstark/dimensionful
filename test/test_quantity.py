...
def test_creation_with_quantity():
    """
    Create a quantity object with a quantity object as ``unit``
    parameter.
    ...
    # array version
    q3 = Quantity(np.arange(4), "Mpc")
    q4 = Quantity(np.arange(4), q3)

    assert (q4.data == np.arange(4)**2).all()
    assert q4.units == u1
